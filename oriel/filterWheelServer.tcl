...
load $env(SPECKLE_DIR)/lib/liboriel.so.tcl85
...
global FILTERWHEEL FWHEELS MAXFILTERS
    ...
    switch [lindex $msg 0] {
        selectfilter { selectfilter [lindex $msg 1] [lindex $msg 2] ; puts $sock "OK" }
        getfilter    { puts $sock "$FWHEELS([lindex $msg 0],position)" }
        reset        { resetFilterWheel $FWHEELS([lindex $msg 0],handle) }
    }
}
