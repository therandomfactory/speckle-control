...
if { [file exists /usr/share/hal/fdi] } {
     ...
     exec sudo cp $env(NESSI_DIR)/config/config/60-camera-andorusb.fdi /usr/share/hal/fdi/information/20thirdparty/.
     exec sudo cp $env(NESSI_DIR)/config/config/60-andorusb-camera-policy.fdi /usr/share/hal/fdi/policy/20thirdparty/.
     exec sudo cp $env(NESSI_DIR)/config/config/andorusb-set-procperm /usr/libexec/.
     set it [tk_dialog .d "andor" "andor HAL support configured" {} -1 OK]
}
if { [file exists /etc/udev/udev.conf] } {
     exec sudo cp $env(NESSI_DIR)/config/config/60-andor.rules /etc/udev/rules.d/.
...
if { [file exists /etc/hotplug/usb.usermap] } {
     ...
     close $fusb
     exec cp $env(NESSI_DIR)/config/config/andor-andoru /etc/hotplug/usb/.
