...
int AndorCancelDriverEvent(int iCardNo)
{
  spin_lock(&gpAndorDev[iCardNo].EventWaitingSpinLock);
  gpAndorDev[iCardNo].EventWaiting = 1;
  spin_unlock(&gpAndorDev[iCardNo].EventWaitingSpinLock);

  wake_up_interruptible(&gpAndorDev[iCardNo].EventWaitingQueue);
